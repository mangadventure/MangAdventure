...
class TestGroup(GroupsTestBase):
    ...
    def test_create(self):
        ...
        assert str(self.group.logo) == 'groups/1/logo.jpg'

    def test_get_directory(self):
        assert str(self.group.get_directory()) == 'groups/1'

    def test_get_absolute_url(self):
        assert str(self.group.get_absolute_url()) == reverse('groups:group',
                                                             kwargs={'g_id': 1})

    def test_members(self):
        assert len(self.group.members.all()) == 0
        member = TestMember.create_member()
        TestRole.create_role(self.group, member)
        TestRole.create_role(self.group, member, 'TS')
        assert len(self.group.members.all()) == 1


class TestMember(GroupsTestBase):
    @staticmethod
    def create_member():
        return Member.objects.create(name='test', twitter='Whatever',
                                     discord='Whatever#8150', irc='user',
                                     reddit='user')
