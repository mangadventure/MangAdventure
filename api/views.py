    ...
    return {
        ...
        'url': request.build_absolute_uri(
            reverse('reader:chapter',
                    args=[_chapter.series.slug, _chapter.volume, _chapter.number])),
        'number': _chapter.number,
        'pages': [request.build_absolute_uri(page.image.url) for page in _chapter.pages.all()],
        'date': _chapter.date,
...
def _series_response(request, _series):
    ...
    for _chapter in _series.chapters.all():
        if _chapter.volume not in response['volumes']:
            response['volumes'][_chapter.volume] = _volume_response(request, _series, _chapter.volume)

    for author in _series.authors.all():
        response['authors'].append(author.name)
        for alias in author.aliases.all():
            response['authors'].append(alias.alias)

    for artist in _series.artists.all():
        response['artists'].append(artist.name)
        for alias in artist.aliases.all():
            response['artists'].append(alias.alias)
...
def chapter(request, slug=None, vol=0, num=0):
    ...
    try:
        _chapter = Chapter.objects.get(series__slug=slug, volume=vol, number=num)
    except ObjectDoesNotExist:
        raise Http404
